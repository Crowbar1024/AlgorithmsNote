...
public:
    vector<vector<int>> combine(int n, int k) {
        vector<vector<int>> res;
        vector<int> subset;
        dfs(n, k, 1, res, subset);
        return res;
    }
    void dfs(int n, int k, int pos, vector<vector<int>>& res, vector<int>& subset) {
        if (subset.size() == k) {
            res.emplace_back(subset);
            return;
        }
        // 现在需要k-subset.size()个元素
        // 那么肯定是从n+1-(k-subset.size())处为当前决策树的这一层的最右节点
        // 而且该节点只能有一个儿子，这个儿子也只有一个儿子……
        for (int i = pos; i <= n+1-k+subset.size(); ++i) {
            subset.emplace_back(i);
            dfs(n, k, i+1, res, subset);
            subset.pop_back();
        }
    }
};

// 90 medium nums中有重复元素，但比如[1,2,2]的子集中不能有两个{1,2}
class Solution {
public:
...
// 40 medium 即39的candidates有重复数字，但比如[1,2,2]凑3不能有两个{1,2}
class Solution {
public:
    vector<vector<int>> res;
    vector<int> subset;
