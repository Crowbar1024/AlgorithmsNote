...
class LRUCache {
    int m_capacity;
    list<pair<int,int>> m_list;  // 一串记录键值对使用时间的链表，越往前越早
    // 为了在O(1)时间内根据key来进行查找和修改，得用hashhap，value就是上述链表中的节点的迭代器
    unordered_map<int,list<pair<int,int>>::iterator> m_map;
public:
    // 如果只是需要提交LRU这个函数，下面的构造函数需要注销
    // LRUCache(int capacity): m_capacity(capacity) {}
    int get(int key) {
        auto it = m_map.find(key);  // it->second才是链表中的一个键值对节点
        if (it == m_map.end()) return -1;
        m_list.splice(m_list.begin(), m_list, it->second);  // 更新链表顺序，把it放到链表头部
    ...
    void put(int key, int value) {
        ...
        if (it != m_map.end()) {  // 更新
            ...
            m_list.splice(m_list.begin(), m_list, it->second);  // 更新it，放到链表头部
        } else {  // 添加
        ...
        if (m_map.size() > m_capacity) {
            int key2delete = m_list.back().first;
            m_list.pop_back();
            m_map.erase(key2delete);
        } 
    }

    vector<int> LRU(vector<vector<int> >& operators, int k) {
        m_capacity = k;
        int times = operators.size();
        vector<int> res;
        for (int i = 0; i < times; i++) {
            int x = operators[i][1], y = operators[i][2];
            if (operators[i][0] == 1) {
                put(x,y);
            } else {
                res.push_back(get(x));
